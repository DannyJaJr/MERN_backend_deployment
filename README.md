...
## You Do: Deploy Your Application

Today, we will be deploying the 'main' branch of your current Application.
If you want to create a test-platform you can deploy the MERN backend developed in class
**Fork and Clone** this repo:
[here](https://github.com/romebell/mern-backend).

#### Note About Branches:

  When you clone a repo, it will clone down the master branch of a given
  repository. However many repos contain branches. Running the command on a
  cloned branch `git checkout solution` would switch the current git branch to
  the `solution` branch. This can be useful for testing out development
  features.
...
- Starts up a new server when we run `heroku create`, and installs all the
  necessary services
- Adds a new remote to our Git repo, so we can just run `git push heroku master` or `git push heroku main`
  to copy our code over to the heroku servers
...
3. From your project directory, create an app on Heroku
   ...
   - Make sure you are in your backend project directory before you run
     this command
...
10. In your Node app's `database.js` file, we want to use the environment to
    ...
    ```diff
    - let connectionString = 'mongodb://localhost/yourDatabaseName'
    ...
    if (process.env.NODE_ENV === "production") {
      ...
      connectionString = "mongodb://localhost/yourDatabaseName";
    }
    ```
    ...
    - In the example above, the link to the MongoDB includes the name of the
      database we are using. When using a
...
13. Add and commit all changes we've made to your application. It is a good idea to push to GH as well. **IMPORTANT** if you used a feature branch, make sure you're making the commit to that branch.
...
15. Set the URI you just copied as an environment variable called `DB_URL` using
    ...
    - Do not copy the a`bo`ve command. The database id numbers in the URI should
...
16. We will be deploying to Heroku from our local the master branch, using the
    ...
    - You can push your code to Heroku remote from other local branches. If you
      ...
      deploy, you would run `$ git push heroku solution:master` in your
      terminal. Heroku detects the source of your `push` and will ignore changes
      from non-main/master local branches

17. Have a seeder file? Seed your Atlas database by running the command:
